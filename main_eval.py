...
def build_hopfield_ensemble(memory, device, beta_override=None):
    ...
    # Selected from the latest ablation: clean-memory ensemble + sum class
    # projection was stronger than augmented/weighted variants. Dot-product
    # branches use beta=20; the Euclidean branch keeps its own smaller scale.
    return [
        ModernHopfieldNetwork(memory, beta=beta(20.0), metric="dot", normalize=True, feature_mode="binary").to(device),
        ModernHopfieldNetwork(memory, beta=beta(20.0), metric="dot", normalize=True, feature_mode="centered").to(device),
        ModernHopfieldNetwork(memory, beta=beta(20.0), metric="dot", normalize=True, feature_mode="binary_centered").to(device),
        ModernHopfieldNetwork(memory, beta=beta(20.0), metric="dot", normalize=True, feature_mode="hybrid_shape").to(device),
        ModernHopfieldNetwork(memory, beta=beta(20.0), metric="dot", normalize=True, feature_mode="profile").to(device),
