...
def save_mchn_memory_artifacts(loader, augmented_memory, augmented_labels, save_dir="./saved_weights"):
    os.makedirs(save_dir, exist_ok=True)
    save_path = os.path.join(save_dir, "mchn_memory_32x64.pt")
    torch.save(
        {
            "image_shape": (64, 32),
            "original_memory_matrix": loader.memory_matrix.cpu(),
            "original_labels": loader.labels.cpu(),
            "augmented_memory_matrix": augmented_memory.cpu(),
            "augmented_labels": augmented_labels.cpu(),
            "idx_to_label": loader.idx_to_label,
            "label_to_idx": loader.label_to_idx,
            "template_paths": loader.template_paths,
            "description": "Modern Hopfield associative memory for polluted license-plate characters.",
        },
        save_path,
    )
