...
=======
            extra_left = 4 if char_position == 0 else 2 if char_position in (1, 6) else 1
            extra_right = 3 if char_position == 0 else 5 if char_position == 6 else 3 if char_position == 1 else 1
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
    ...
    def _refine_geometry_boxes_by_projection(binary, boxes):
        ...
        for idx in range(len(refined) - 1):
            ...
            local_w = max(4, int(0.06 * min(left_box[2], right_box[2])))
            ...
            if reference <= 0 or valley_value > max(1.2, 0.38 * reference):
                continue
            if abs(valley - expected) > max(3, int(0.05 * min(left_box[2], right_box[2]))):
                continue
            min_gap = 2
            ...
            if left_new_w < 0.90 * original[idx][2] or right_new_w < 0.90 * original[idx + 1][2]:
                continue
...
=======
            pad_x = max(3, int(round(w * 0.080)))
        elif position == 1:
            pad_x = max(2, int(round(w * 0.080)))
        elif position == 6:
            pad_x = max(3, int(round(w * 0.090)))
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
        else:
...
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
        ...
        for cnt in contours:
            ...
            if position is not None and position >= 1:
                if y <= 0.07 * h_img and h <= 0.14 * h_img and area < 0.040 * h_img * w_img:
                    continue
                if y + h >= 0.93 * h_img and h <= 0.14 * h_img and area < 0.035 * h_img * w_img:
                    continue
...
=======
            if position in {0, 6} and h >= 0.97 * h_img and w <= 0.035 * w_img and (x <= 1 or x + w >= w_img - 1):
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
                continue
...
=======
            if position == 0 and touches_left and h >= 0.92 * h_img and w <= 0.07 * w_img and area < 0.055 * h_img * w_img:
                cv2.drawContours(cleaned, [cnt], -1, 0, thickness=-1)
            elif position == 0 and touches_right and h >= 0.82 * h_img and w <= 0.08 * w_img and area < 0.045 * h_img * w_img:
                cv2.drawContours(cleaned, [cnt], -1, 0, thickness=-1)
            elif position == 6 and touches_right and h >= 0.92 * h_img and w <= 0.08 * w_img and area < 0.055 * h_img * w_img:
                cv2.drawContours(cleaned, [cnt], -1, 0, thickness=-1)
            elif position in {0, 6} and (touches_left or touches_right) and h >= 0.94 * h_img and w <= 0.045 * w_img and area < 0.040 * h_img * w_img:
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
                cv2.drawContours(cleaned, [cnt], -1, 0, thickness=-1)
    ...
    def _remove_slot_rivets_and_reflections(mask, position=None):
        ...
        for cnt in contours:
            ...
            near_top = y <= 0.07 * h_img
            near_bottom = y + h >= 0.93 * h_img
...
=======
            side_line = (touches_left or touches_right) and h >= 0.90 * h_img and w <= 0.08 * w_img and area < 0.055 * h_img * w_img
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
            edge_speck = (touches_left or touches_right) and area < 0.020 * h_img * w_img
            cap_line = (near_top or near_bottom) and w >= 0.55 * w_img and h <= 0.10 * h_img
...
=======
                inner_left = x + int(0.006 * w)
                inner_right = x + w - int(0.018 * w)
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
...
=======
                if touches_left and h >= 58 and w <= 2 and area < 0.035 * 64 * 32:
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
...
=======
                elif touches_right and w <= 3 and h >= 54 and area < 0.035 * 64 * 32:
                    cv2.drawContours(remove_mask, [cnt], -1, 255, thickness=-1)
            elif position == 6:
                if touches_right and w <= 3 and h >= 56 and area < 0.035 * 64 * 32:
                    cv2.drawContours(remove_mask, [cnt], -1, 255, thickness=-1)
                elif touches_left and w <= 2 and h >= 56 and area < 0.030 * 64 * 32:
>>>>>>> 7f66310ecdc44187029d6a88fc518fc4a94cb153
                    cv2.drawContours(remove_mask, [cnt], -1, 255, thickness=-1)
                elif (y <= 2 or y + h >= 62) and w >= 22 and h <= 4:
                    cv2.drawContours(remove_mask, [cnt], -1, 255, thickness=-1)
            else:
                if (y <= 2 or y + h >= 62) and w >= 24 and h <= 4:
                    cv2.drawContours(remove_mask, [cnt], -1, 255, thickness=-1)
    ...
    def _foreground_gray_signal(gray_crop, mask=None, position=None):
        ...
        if mask is not None and mask.size:
            ...
            if 0.035 <= float(np.mean(dilated > 0)) <= 0.78:
                ...
                support_kernel = (9, 5) if position in {0, 6} else (15, 5)
                support = cv2.bitwise_or(
                    dilated,
                    cv2.bitwise_and(bright_support, cv2.dilate(dilated, cv2.getStructuringElement(cv2.MORPH_RECT, support_kernel))),
                )
                if position not in {0, 6}:
                    ys, xs = np.where(dilated > 0)
                    if len(xs) > 0 and len(ys) > 0:
                        h_img, w_img = enhanced.shape[:2]
                        pad_x = max(2, int(0.22 * (xs.max() - xs.min() + 1)))
                        pad_y = max(1, int(0.08 * (ys.max() - ys.min() + 1)))
                        roi = np.zeros_like(support)
                        x1 = max(0, int(xs.min()) - pad_x)
                        x2 = min(w_img, int(xs.max()) + pad_x + 1)
                        y1 = max(0, int(ys.min()) - pad_y)
                        y2 = min(h_img, int(ys.max()) + pad_y + 1)
                        roi[y1:y2, x1:x2] = 255
                        support = cv2.bitwise_or(support, cv2.bitwise_and(bright_support, roi))
                signal[support == 0] = 0
        else:
        ...
        if contours:
            ...
            for cnt in contours:
                ...
                if position in {0, 6} and (touches_left or touches_right) and area < 0.010 * h_img * w_img:
                    continue
                if (y <= 0.04 * h_img or y + h >= 0.96 * h_img) and w >= 0.55 * w_img and h <= 0.10 * h_img:
                    continue
                cv2.drawContours(kept, [cnt], -1, 255, thickness=-1)
            signal[kept == 0] = 0
        return signal
